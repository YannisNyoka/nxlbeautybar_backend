...
// (admin-blocked or occupied by a confirmed appointment) between startDate
// and endDate inclusive, for the given employee ('any' = all staff).
...
//
// Only appointments with status booked/completed/no-show AND paymentStatus
// deposit_paid/paid count as occupying a slot — this matches the DB's own
// unique index (APPOINTMENTS' partialFilterExpression in initCollections.js)
// and the conflict checks in crudRoutes/check-availability. A pending,
// unpaid booking should never hold a slot against other customers —
// abandoned checkouts must not block the calendar.
