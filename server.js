...
const fetchFn = require('./lib/fetchPolyfill');

const { pad2, normalizeTimeTo24h, normalizeDateToISO, normalizeAppointmentDateTime, generateSlotRange } = require('./lib/dateTime');
...
async function startServer() {
  try {
    ...
    const { notifyClient, NOTIF_ICONS } = require('./lib/notifications')(db);
    const { LOYALTY_CONFIG, getLoyaltyAccount, calcTier, awardPoints, redeemPoints } = require('./lib/loyalty')(db, notifyClient);
...
${urlEntries}
    ...
    const REFERRAL_CONFIG = require('./lib/referrals-config');
